...
async function requestXml(endpoint,params,label){
  const rawKey=process.env.DATA_GO_KR_SERVICE_KEY?.trim(), decodedKey=normalizeServiceKey(rawKey);
  if(!decodedKey)throw new Error('공공데이터 인증키가 설정되지 않았습니다.');
  const makeUrl=(key)=>{const u=new URL(endpoint);u.searchParams.set('serviceKey',key);Object.entries(params).forEach(([k,v])=>u.searchParams.set(k,String(v)));return u};
  let res=await fetch(makeUrl(decodedKey),{cache:'no-store',headers:{Accept:'application/xml,text/xml,*/*'}}), text=await res.text();
  if((!res.ok||/SERVICE_KEY_IS_NOT_REGISTERED_ERROR|등록되지 않은 서비스키|returnReasonCode[^0-9]*30/i.test(text))&&rawKey&&rawKey!==decodedKey){
    const q=Object.entries(params).map(([k,v])=>`${encodeURIComponent(k)}=${encodeURIComponent(String(v))}`).join('&');
    res=await fetch(`${endpoint}?serviceKey=${rawKey}&${q}`,{cache:'no-store',headers:{Accept:'application/xml,text/xml,*/*'}});text=await res.text();
  }
  if(!res.ok)throw new Error(`${label} HTTP ${res.status}: ${text.slice(0,300)}`);
  const xml=parser.parse(text), h=findHeader(xml);
  if(h?.code==='4')return{xml,noResult:true};
  if(h?.code&&!['0','00','1'].includes(h.code))throw new Error(`${label} API 오류 ${h.code}: ${h.msg||'원인 미상'}`);
  return{xml,noResult:false};
}
...
export async function GET(){
  ...
  return NextResponse.json({ok:hard.length<all.length,partial:hard.length>0||locErr.length>0,live:true,source:'인천광역시 버스정보시스템(BIS)',version:'0.13',method:'첫 버스=도착정보 API, 현재/다음 버스 위치=버스위치정보 API',commute,returnHome,locationApi:{ok:locErr.length===0,errorCount:locErr.length},fetchedAt:new Date().toISOString(),elapsedMs:Date.now()-startedAt},{headers:{'Cache-Control':'no-store, max-age=0'}});
}
